...
mujoco_engine::mujoco_engine(){
    ...
    mujoco_data = make_unique<mujoco_data_>();
}
...
void mjModel_Deleter::operator()(mjModel * m){
    if(m)
        mj_deleteModel(m);
}


void mjData_Deleter::operator()(mjData * d){
    if(d)
        mj_deleteData(d);
}


void GLFWwindow_Deleter::operator()(GLFWwindow_ * w){
    if(w)
        glfwDestroyWindow(w);
}